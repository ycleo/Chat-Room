	...
	{  
	    ...
	    {  
			...
			case FD_CLOSE:  // 遠端斷線事件
				...
				for (int i = U; i < No+1; i++) {
					m_List1.SetItemText(i, 0, m_List1.GetItemText(i + 1, 0));
					m_List1.SetItemText(i, 1, m_List1.GetItemText(i + 1, 1));
					m_List1.SetItemText(i, 2, m_List1.GetItemText(i + 1, 2));
				}

				UpdateData(1); 
				n.Format(L"IP: 127.0.0.1 / wParam %d 第%d列  遠端斷線, %d \r\n", wParam, U, No);
				m_Edit1 += n;
